      ...
      <main className="flex-1 p-4 md:p-6 flex items-start justify-center overflow-y-auto">
          ...
          <div className="text-center max-w-4xl w-full p-8 bg-card/30 border border-border/80 rounded-3xl backdrop-blur-md shadow-2xl">
            ...
            <div className="mt-10 border-t border-border pt-8 text-left">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-bold text-muted-foreground uppercase tracking-wider">Combatentes Conectados</h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                {students.map((s, i) => (
                  <div key={i} className="px-4 py-3 bg-background/60 border border-border/80 rounded-xl text-base md:text-lg font-bold text-foreground flex items-center gap-3 shadow-sm animate-in fade-in zoom-in duration-300">
                    {s.avatarUrl ? (
          ...
          <div className="max-w-[95vw] w-full flex flex-col lg:flex-row gap-6">
            ...
            <div className="flex-1 space-y-6 flex flex-col">
              {currentQuestionIndex === -1 ? (
                ...
                <div className="flex-1 bg-card border border-border rounded-2xl p-5 md:p-8 flex flex-col">
                  <div className="flex justify-between items-center mb-6">
                              ...
                              {isEnded && questionEndedData.percentages && (
                                <div className="pl-14 flex items-center gap-4 w-full animate-in fade-in slide-in-from-left-2 duration-300">
                                  <Progress 
                                    ...
                                    className={`h-2.5 flex-1 bg-background ${isCorrect ? '[&>div]:bg-emerald-500' : '[&>div]:bg-blue-500/80'}`} 
                                  />
                                  <span className={`text-sm md:text-lg font-black font-mono w-14 text-right ${isCorrect ? 'text-emerald-400' : 'text-muted-foreground'}`}>
            ...
            <div className="w-full lg:w-80 xl:w-96 flex flex-col gap-6 shrink-0">
              ...
              {!isQuestionActive && questionEndedData && (
                <Card className="bg-card border-emerald-900/50 shadow-xl">
                    ...
                    {teams.map((team: any, tIdx: number) => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between border-b border-border/60 pb-2">
                            ...
                            <div className="text-right">
                              <div className="font-mono font-black text-lg text-emerald-400">
                                {team.totalScore || 0} <span className="text-xs font-bold text-muted-foreground">pts</span>
                              </div>
                          ...
                          {members.length === 0 ? (
                            <div className="text-xs text-muted-foreground italic py-1 text-center font-medium">
                                ...
                                >
                                  <div className="flex items-center gap-2 overflow-hidden">
                                    {member.avatarUrl ? (
                                      ...
                                      <div className="w-5 h-5 rounded-full bg-muted text-[9px] font-bold text-muted-foreground flex items-center justify-center shrink-0">
                                        {member.name?.substring(0, 2).toUpperCase()}
                                      </div>
                                    )}
                                    <span className="text-foreground font-bold truncate">{member.name}</span>
                                  ...
                                  <select
                                    ...
                                    className="bg-card text-[10px] font-bold text-muted-foreground border border-border rounded px-1.5 py-0.5 focus:outline-none focus:border-emerald-500 cursor-pointer"
                                  >
                                    {teams.map((t: any) => (
              ...
              {/* Ranking Ao Vivo */}
              <Card className="bg-card border-border flex-1 flex flex-col shadow-2xl">
                <CardHeader className="border-b border-border py-3 md:py-4">
                  <CardTitle className="text-foreground flex items-center gap-2 font-bold text-sm md:text-base">
                            ...
                            {(() => {
                              ...
                              return (
                                <div className={`shrink-0 rounded-full ${p.avatarRing || ''}`}>
                                  {aluno.avatarUrl ? (
                                    <img src={aluno.avatarUrl} alt="Avatar" className="w-7 h-7 md:w-9 md:h-9 rounded-full object-cover border border-border shrink-0" />
                                  ) : (
                                    <div className="w-7 h-7 md:w-9 md:h-9 rounded-full bg-muted text-muted-foreground flex items-center justify-center text-[10px] md:text-xs font-bold shrink-0 border border-border">
        ...
        {status === "FINISHED" && (
          <div className="text-center max-w-2xl w-full animate-in fade-in zoom-in duration-500">
            ...
            {isTeamCompetition && teams.length > 0 && (
              <div className="w-full bg-card/60 border border-emerald-500/50 rounded-2xl p-6 mb-8 shadow-[0_0_30px_rgba(16,185,129,0.15)]">
            ...
            {/* Podium Top 3 */}
            {ranking.length > 0 && (
              <div className="w-full bg-card/50 border border-border rounded-xl p-6 mb-8">
                <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-widest mb-6">Top 3 Combatentes</h3>
                <div className="flex justify-center items-end gap-4 h-48">
                  ...
                  {ranking.length > 0 && (
                    <div className="flex flex-col items-center w-32">
                      ...
                      <div className="w-16 h-16 rounded-full border-2 border-yellow-500 mb-3 overflow-hidden bg-muted flex items-center justify-center shadow-[0_0_15px_rgba(234,179,8,0.3)]">
                        {ranking[0].avatarUrl ? (
                  ...
                  {ranking.length > 2 && (
                    <div className="flex flex-col items-center w-28">
                      <div className="w-12 h-12 rounded-full border-2 border-amber-700 mb-3 overflow-hidden bg-muted flex items-center justify-center">
                        {ranking[2].avatarUrl ? (
                      ...
                      <div className="text-sm text-muted-foreground truncate w-full text-center">{ranking[2].name.split(' ')[0]}</div>
              ...
              return (
                <div className="flex flex-col gap-2">
                  {list.map((aluno: any, idx: number) => (
                    <div key={idx} className="flex items-center gap-3 p-2 bg-background/40 rounded-lg border border-border/60">
                      {aluno.avatarUrl ? (
