...
export default function InventoryClient({ role, user, onBack }: InventoryClientProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-[0_0_30px_rgba(0,0,0,0.5)]">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div className="p-3 bg-gradient-to-br from-blue-600 to-indigo-600 rounded-xl shadow-[0_0_20px_rgba(37,99,235,0.4)]">
              <Package className="w-7 h-7 text-white" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-2xl font-black text-white tracking-wide uppercase">
                  Inventário da Sala - 32º Pelotão
                </h1>
                <span className={`text-[10px] px-2 py-0.5 rounded font-black uppercase tracking-wider ${role === 'INSTRUCTOR' ? 'bg-blue-600 text-white' : 'bg-emerald-600 text-white'}`}>
                  {role === 'INSTRUCTOR' ? 'Acesso Instrutor' : 'Acesso Combatente'}
                </span>
              </div>
              <p className="text-sm text-slate-400 font-medium">
                Controle militar em tempo real e auditoria de alterações da sala do pelotão
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <button
            onClick={fetchAll}
            disabled={loading}
            className="px-4 py-3 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white rounded-xl font-bold text-xs uppercase tracking-wider flex items-center gap-2 transition-all disabled:opacity-50"
          >
        ...
        <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4">
          <div className="flex bg-slate-950 p-1.5 rounded-xl border border-slate-800">
            <button
              ...
              className={`flex-1 sm:flex-initial px-6 py-2.5 rounded-lg font-black text-xs uppercase tracking-wider transition-all flex items-center justify-center gap-2 ${
            ...
            >
              ...
              Histórico de Alterações ({histories.length})
            </button>
          </div>
          ...
          {activeTab === "table" && (
            <div className="flex flex-wrap items-center gap-3">
              ...
              <select
                ...
                className="bg-slate-950 border border-slate-800 text-slate-300 rounded-xl px-3 py-2.5 text-xs font-bold focus:outline-none focus:border-blue-500"
              >
